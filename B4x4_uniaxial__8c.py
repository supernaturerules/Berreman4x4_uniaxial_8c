...
Provides
  ...
  reflectance spectra of chiral nematic stacks in the presence of a uniaxial layer
...
Author
  Yitao Xu (supernature.rules@gmail.com)
"""
...
class Simulator(object):
    ...
    Properties
      ...
      c:           thickness of the top uniaxial layer (nm)
      a:            coefficient for retardance dispersion law (a/lbda)
      b:            coefficient for retardance dispersion law (b/lbda^3)
    ...
    def __init__(self):
        # Added 'c', 'a', 'b' to settings
        self.settings = dict(no=1.524, ne=1.586, nSubstrate=1.59, nSuperstrate=1, pitch=162, stack=12, angle=0,
                             lbda_min=400,
                             lbda_max=800, points=101, slices=23,
                             c=525000, a=1520.01054, b=25345000)
    ...
    def prepare(self):
        ...
        # Inhomogeneous layer, repeated layer (The Stack)
        IL = Berreman4x4.InhomogeneousLayer(TN)
        L_stack = Berreman4x4.RepeatedLayers([IL], self.settings['stack'])
        ...
        for lbda in lbda_list:
            ...
            # Calculate dispersion for the top layer
            # Law: (a / lbda + b / lbda^3) / 2 / pi
            
            a = self.settings.get('a', 0)
            b = self.settings.get('b', 0)
            c = self.settings.get('c', 0)
            
            # Avoid division by zero
            if lbda == 0: lbda = 1e-9

            # Calculate retardance/delta_n term
            ret_val = (a / lbda + b / (lbda ** 3)) 
            delta_n_top = ret_val * lbda / (2 * pi * c)

            Create the Top Uniaxial Layer
            # no = 1.3, ne = 1.3 + delta_n
            no_top = 1.3
            ne_top = 1.3 + delta_n_top
            
            TopMat = Berreman4x4.UniaxialNonDispersiveMaterial(no_top, ne_top)
            
            # Rotate top layer to be in-plane (ne along x) similar to the LC start
            # If you need a specific azimuth for the retarder, rotate around Z here.
            TopMat = TopMat.rotated(Berreman4x4.rotation_v_theta(Berreman4x4.e_x, pi / 2)) 
            
            # Build Structure
            
            layer_list = [L_stack]
            if c > 0:
                # Assuming HomogeneousLayer exists for a slab of constant material
                
                
                try:
                    TopLayer = Berreman4x4.HomogeneousLayer(TopMat, c, hs_method="Padé", hs_order=3)
                except AttributeError:
                    
                    TopFlat = Berreman4x4.TwistedMaterial(TopMat, c, angle=0, div=1)
                    TopLayer = Berreman4x4.InhomogeneousLayer(TopFlat)
                
                
                layer_list.insert(0, TopLayer)

            s = Berreman4x4.Structure(front, layer_list, back)

            # Calculate Jones Matrix for this wavelength
            J_now = s.getJones(sin(self.settings['angle'] * pi / 180), k0)
            J_list.append(J_now)
    ...
    # --- New Properties for easier access ---
    @property
    def c(self):
...
if __name__ == '__main__':
    mySim = Simulator()
    # Example usage with new layer:
    # mySim.c = 1000  # thickness in nm
    # mySim.a = 500    # dummy dispersion Coeff
    mySpec = mySim.calculateR()
    mySpec.plot()
